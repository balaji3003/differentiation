...
def get_first_addition_of_ci_configs():
    ...
    log_data = run_git_command("git log --since='10 years ago' --diff-filter=A --name-only --pretty=format:'%h|%ad|%an|%s' --date=short")
...
def detect_test_commit_spikes():
    ...
    log_data = run_git_command("git log --since='10 years ago' --pretty=format:'%ad %s' --date=short")
...
def save_output_to_file():
    ...
    output_filename = os.path.join(output_dir, f"{repo_name}_test_adoption_{timestamp}.txt")
